...
Graph GraphMST(Graph g) {
    ...
    Graph new = GraphNew(GraphNumVertices(g));
    // Loading all values into priority queue
    PQ pq = PQNew();
    for (int i = 0; i < GraphNumVertices(g); i++) {
        for (int j = 0; j < GraphNumVertices(g); j++) {
            if (g->edges[i][j] != 0) {
                ...
                newE.v = i;
                newE.w = j;
                newE.weight = g->edges[i][j];
                PQInsert(pq, newE);
            }
        }
    }
    
    // While not enough edges and queue isn't empty
    while (new->nE != new->nV - 1 && !PQIsEmpty(pq)) {
        // Get the lowest value avaliable edge
        Edge u = PQExtract(pq);
        // Insert this edge
        GraphInsertEdge(new, u);
        // If there is a cycle, remove the edge
        if (GraphHasCycle(new)) {
            GraphRemoveEdge(new, u.v, u.w);
        }
    }
    
    // Check if MST exists, if not return NULL
    if (new->nE != new->nV - 1) {
        GraphFree(new);
        PQFree(pq);
        return NULL;
