...
void BSTreeLevelOrder(BSTree t) {
	...
	BSTNode *temp_node = t;
	QueueEnqueue(new_queue, temp_node);
    //While the queue still has entries
	while (!QueueIsEmpty(new_queue)) {
	    //Get the value at the front and print it 
	    temp_node = QueueFront(new_queue);
	    printf("%d ", temp_node->value);
	    //Add its left child
	    if (temp_node->left != NULL) {
	        QueueEnqueue(new_queue, temp_node->left);
	    }
	    //Add its right children
	    if (temp_node->right != NULL) {
	        QueueEnqueue(new_queue, temp_node->right);
        }
