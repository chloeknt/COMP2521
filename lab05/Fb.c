...
bool FbUnfriend(Fb fb, char *name1, char *name2) {
    ...
    int id2 = nameToId(fb, name2);
    assert(id1 != id2);
    // Check if friends, if so, 'unfriend'
    if (fb->friends[id1][id2]) {
...
List FbMutualFriends(Fb fb, char *name1, char *name2) {
    ...
    List l = ListNew();
    for (int i = 0; i < fb->numPeople; i++) {
        //Append list of mutuals
        if (fb->friends[id1][i] && fb->friends[i][id2]) {
...
void FbFriendRecs1(Fb fb, char *name) {
    ...
    //Max is n - 2 mutuals
    for (int n = fb->numPeople - 2; n > 0; n--) {
        for (int i = 0; i < fb->numPeople; i++) {
            //Not friends or themself
            if (!fb->friends[id][i] && strcmp(name, fb->names[i]) != 0) {
                //Obtain a list of mutuals
                l = FbMutualFriends(fb, name, fb->names[i]);
                if (ListSize(l) == n) {
                    printf("\t%-20s%4d mutual friends\n", fb->names[i], ListSize(l));
                }
            }
        }
    }
