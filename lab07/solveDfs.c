...
bool dfsPath(Maze m, Cell **c, Cell **pred, bool **visited, int row, int col);
...
bool solve(Maze m) {
    ...
    if (dfsPath(m, c, pred, visited, enter.row, enter.col)) {
...
bool dfsPath(Maze m, Cell **c, Cell **pred, bool **visited, int row, int col) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (valid(m, c, visited, newRow, newCol)) {
            ...
            } else if (dfsPath(m, c, pred, visited, newRow, newCol)) {
                return true;
            }
        }
    }
